...
import { ChevronRight, ShieldCheck, Sparkles, User, UserCog } from "lucide-react";
...
export default function LandingPage() {
  ...
  const handleSignIn = async () => {
    setLoading(true);
    // Save the selected role in a cookie so the backend knows which role was chosen
    document.cookie = `selectedRole=${role}; path=/; max-age=3600`;
    await signIn("google");
  };

  const handleDemoSignIn = async (e: any) => {
    e.preventDefault();
    setLoading(true);
    document.cookie = `selectedRole=${role}; path=/; max-age=3600`;
    await signIn("credentials", { email: demoEmail, password: demoPassword });
  };
        ...
        >
          ...
          <div className="bg-white px-10 py-12 rounded-[2rem] text-center shadow-[0_10px_50px_rgba(0,0,0,0.05)] border border-slate-100">
            ...
            <motion.div 
              ...
              className={`mx-auto bg-${role === 'ADMIN' ? 'violet-100 text-violet-600' : 'blue-100 text-blue-600'} p-4 rounded-full w-20 h-20 flex items-center justify-center mb-6 transition-colors duration-500`}
            >
               {role === 'ADMIN' ? <UserCog className="w-10 h-10" /> : <User className="w-10 h-10" strokeWidth={1.5} />}
            </motion.div>
            ...
            <p className="text-slate-500 mb-10 text-sm px-2 leading-relaxed">
              Welcome back. Please specify your role to sign into the housing administration system.
            </p>
            ...
            <div className="flex bg-slate-100 p-1.5 rounded-xl mb-8 relative z-20">
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-2 py-3 px-4 rounded-lg text-sm font-semibold transition-all duration-300 relative ${
                  role === "STUDENT" ? "text-blue-700 bg-white shadow-sm" : "text-slate-500 hover:text-slate-700"
                }`}
              >
              ...
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-2 py-3 px-4 rounded-lg text-sm font-semibold transition-all duration-300 relative ${
                  role === "ADMIN" ? "text-violet-700 bg-white shadow-sm" : "text-slate-500 hover:text-slate-700"
                }`}
              >
              ...
              {!demoMode ? (
                ...
                >
                  <button
                    ...
                    className={`w-full text-white font-medium py-4 px-6 rounded-xl flex items-center justify-center gap-4 transition-all duration-300 shadow-md hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed ${role === 'ADMIN' ? 'bg-violet-600 hover:bg-violet-700' : 'bg-blue-600 hover:bg-blue-700'}`}
                  >
                    {loading || status === "loading" ? (
                      ...
                      <>
                        <div className="bg-white p-1 rounded-md">
                            <svg viewBox="0 0 24 24" className="w-4 h-4 flex-shrink-0" preserveAspectRatio="xMidYMid meet">
            ...
            <div className="mt-8 flex items-center justify-center gap-2 text-xs text-slate-400 font-medium">
              ...
              <button 
                ...
                className="ml-auto text-slate-300 hover:text-slate-500 font-bold transition-colors cursor-pointer"
