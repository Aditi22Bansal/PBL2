...
def compute_allocation(profiles_dict, config=None):
    """
    Core allocation orchestration logic (bucket by gender/branch/year, claim rooms
    from the global inventory, run greedy matching per bucket, aggregate metrics).

    This is a pure extraction of what used to live inline in run() below - no
    matching/scoring logic was changed, only the I/O boundary (it now takes plain
    args and returns a dict instead of reading stdin / printing to stdout). That
    lets the exact same code be called from both the CLI entrypoint (run()) and
    the FastAPI /allocate/v2 route in main.py.
    """
    if config is None:
