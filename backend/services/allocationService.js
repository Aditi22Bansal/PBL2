...
const runRelaxedPythonAllocation = async (profiles) => {
    try {
        const pythonBackendUrl = process.env.PYTHON_BACKEND_URL || 'http://127.0.0.1:8000';
        const response = await axios.post(`${pythonBackendUrl}/api/allocate-relaxed`, profiles);
        return response.data;
    } catch (error) {
        console.error('Python Relaxed API Error:', error.response?.data || error.message);
        throw new Error(`Failed to run relaxed allocation: ${error.response?.data?.detail || error.message}`);
    }
};
